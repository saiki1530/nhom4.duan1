    ...
    <body>
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'edit') {
            if (isset($_POST['id']) && !empty($_POST['id']) && isset($_POST['password']) && !empty($_POST['password'])) {
                $result = mysqli_query($con, "UPDATE `user` SET `password` = MD5('" . $_POST['password'] . "')  WHERE `user`.`id` = " . $_POST['id'] . ";");
                if (!$result) {
                ...
                <div id="edit_user" class="box-content">
                    ...
                    <form action="./edit_user.php?action=edit" method="Post" autocomplete="off">
                        ...
                        <select name="status">
                            ...
                            <option <?php if (empty($user['status'])) { ?> selected <?php } ?>  value="0">khóa</option>
                        </select>
                        <br><br>
